...
uint256 hashGenesisBlock(HASHGENESISBLOCK);
...
static const int64 nTargetTimespan = NTARGETTIMESPAN * 24 * 60 * 60; // 
static const int64 nTargetSpacing = NTARGETSPACING; // SmallChange: 15 seconds
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal caticoin miner
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256(HASHGENESISBLOCK_TESTNET);
    }
	...
	// vMerkleTree: 5a2e19825b
        ...
        block.nVersion = 1;
        block.nTime    = EPOCTIME;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = NNONCE;

        if (fTestNet)
        {
            block.nTime    = EPOCTIME_TESTNET;
            block.nNonce   = NNONCE;
        }
        ...
        assert(block.hashMerkleRoot == uint256(HASHMERKELROOT));

        // If genesis block hash does not match, then generate new genesis hash.
        if (true && block.GetHash() != hashGenesisBlock) //from_false_to_true
        {
